...
def search_youtube_videos(api_key, query, max_results=50, page_token=None):
...
def main(api_key, query, max_results=50, filename="processed_video_ids.txt", output_file="C:/Users/soo/Desktop/soo/2024/env_capstone/youtubeAPI_list.xlsx"):
    ...
    while True:
        # "환경" 키워드로 검색
        videos = search_youtube_videos(api_key, query, max_results, next_page_token)
        
        # 새로운 동영상 필터링
        new_videos = [item for item in videos['items'] if item['id']['videoId'] not in processed_video_ids and item['id']['videoId'] not in existing_video_ids]

        if not new_videos:
            break  # 새로운 동영상이 없으면 종료
